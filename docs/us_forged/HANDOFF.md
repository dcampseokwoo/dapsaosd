...
1. **[신규·최우선] 상장사/대기업 계열이 발송 리스트에 혼입 — 스테이지 데이터 오류.**
   §3/v4로 안 잡힘(분류가 아니라 스테이지 값 문제). 실측:
   - **휴젤(주)**: 코스닥 상장 보톡스 대기업인데 DB 스테이지가 **"Seed"로 오기** → 통과.
     스테이지 값 자체가 신뢰 불가.
   - 스테이지 "알 수 없음" 255건에 상장/대형 의심 다수: 알테오젠바이오로직스(알테오젠 계열)·
     녹십자수의약품(GC 계열)·로킷헬스케어·세미파이브·알피니언메디칼·대한조선·창명해운(해운)·
     테크로스·콘텔라 등, 그리고 기성 제조 중소기업 클러스터(서진산업·신영금속·우창공업·
     대성철강·광명산업·삼포산업·범한산업·우리공업·전우정밀 — 대부분 maturity_signal 플래그).
   - **처리 방향 미정**(사용자와 정할 것): 별도 배제 규칙(설립연도·직원수·상장 여부 외부
     데이터 필요, DB엔 없음) vs 플래그(established/listed_suspect) vs 설문 자기배제.
     현 구조상 DB로 검증 불가라 설문 단계가 자연 필터.
