...
`확인 필요` 축의 처리 방식이 프롬프트에서 충돌하므로 세 모드를 병행 계산한다.
`strict`(증거 없으면 하위 레벨) / `neutral`(운영원칙1대로 제외 후 재정규화) / `v2`(수정판).

v1 은 실제 합격 4개사 중 3개사를 오탈락시킨다. 원인은 ① 절대 레벨표 탓에 프리시드가
Traction L2 상한에 갇히는 구조, ② 정보 부재를 L1/L3 으로 환산하는 처리, ③ 커버리지
개념 부재, ④ HAX 와 충돌하는 강등 규칙. v2 는 가중치를 그대로 둔 채 이 넷만 고쳐
오탈락 0개사 · 통과율 31% 를 낸다. 상세는 `screening/RESULTS.md` 2-1절.

v2 는 Fit 도 규칙표(6신호 가중합)로 계산해 마지막 정성 판단 구간을 닫고,
Quality × Fit → **조치**(추천 진행 / 설문 요청 / 타 프로그램 / 탈락 / 에스컬레이션 /
라우팅)까지 확정한다. `판정 보류`는 탈락이 아니라 설문·증빙 요청 상태이며,
Fit 낮음 × 보류는 설문 발송 대상에서 제외한다(Fit 은 공개 신호로 이미 확정되므로).
재평가 결과는 `screening/REEVALUATION.md`.
