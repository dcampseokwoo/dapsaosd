...
from screening import uf_diff, uf_snapshot

SNAP = uf_snapshot.DEFAULT_SNAPSHOT


def test_resolve_snapshot_picks_frozen():
    assert uf_snapshot.resolve_snapshot().name.startswith("GBD_DB_")


def test_self_diff_is_empty():
    rows = uf_snapshot.load_rows(SNAP)
    d = uf_diff.column_diff(rows, rows)
...
def test_candidate_impact_deferred_without_pipeline():
    with pytest.raises(NotImplementedError):
        uf_diff.candidate_impact([], [], shortlist_fn=None)


def test_candidate_impact_with_injected_shortlist():
    old = [{"biz_no": "1"}, {"biz_no": "2"}]
    new = [{"biz_no": "2"}, {"biz_no": "3"}]
    imp = uf_diff.candidate_impact(old, new, shortlist_fn=lambda rs: {r["biz_no"] for r in rs})
