...
"""
from screening import uf_dedup
...
def test_one_digit_diff_helper():
    assert uf_dedup.is_one_digit_diff("661-81-02253", "667-81-02253")   # 아폴론
    assert uf_dedup.is_one_digit_diff("367-87-02674", "364-87-02674")   # 크로스포인트
    assert not uf_dedup.is_one_digit_diff("563-88-23981", "563-88-02981")  # 오믈렛=2자리차
    assert not uf_dedup.is_one_digit_diff("661-81-02253", "661-81-02253")  # 동일


def test_norm_name_strips_legal_forms():
    assert uf_dedup._norm_name("주식회사 워커린스페이스") == uf_dedup._norm_name("워커린스페이스")
    assert uf_dedup._norm_name("(주)티아이") == uf_dedup._norm_name("티아이")
    assert uf_dedup._norm_name("(주)유티아이") != uf_dedup._norm_name("티아이")


def test_same_biz_different_spelling_merges():
    """워커린스페이스: 동일 사업자번호인데 표기만 다름 → 1개 엔티티."""
    ents = uf_dedup.resolve_entities(_rows(
...
def test_one_digit_diff_merges():
    ...
    ap = uf_dedup.resolve_entities(_rows(
